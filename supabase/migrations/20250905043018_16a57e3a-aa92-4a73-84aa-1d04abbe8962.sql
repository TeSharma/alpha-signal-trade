-- Hybrid architecture Phase 1 DB changes - fixed version
...
-- Safely add updated_at column and trigger if they don't exist
DO $$ 
BEGIN
...
-- Drop and recreate RLS policies for deposits
DROP POLICY IF EXISTS "Users can view their own deposits" ON public.deposits;
CREATE POLICY "Users can view their own deposits"
ON public.deposits FOR SELECT
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can create their own deposit records" ON public.deposits;
CREATE POLICY "Users can create their own deposit records"
ON public.deposits FOR INSERT
WITH CHECK (auth.uid() = user_id AND status = 'pending');

-- Add trigger for deposits
DO $$ BEGIN
...
DROP POLICY IF EXISTS "Users can view their own bridge txs" ON public.bridge_transactions;
CREATE POLICY "Users can view their own bridge txs"
ON public.bridge_transactions FOR SELECT
USING (initiated_by IS NOT NULL AND auth.uid() = initiated_by);

DROP POLICY IF EXISTS "Users can create their own bridge txs" ON public.bridge_transactions;
CREATE POLICY "Users can create their own bridge txs"
ON public.bridge_transactions FOR INSERT
WITH CHECK (initiated_by IS NOT NULL AND auth.uid() = initiated_by);

-- Add trigger for bridge transactions
DO $$ BEGIN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'trades_chain_values') THEN
    ALTER TABLE public.trades
